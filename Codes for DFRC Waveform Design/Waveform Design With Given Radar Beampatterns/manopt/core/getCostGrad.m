...
% function [cost, grad] = getCostGrad(problem, x, storedb, key)
%
% Returns the value at x of the cost function described in the problem structure, as well as the gradient at x.
%
% storedb is a StoreDB object, key is the StoreDB key to point x.
% See also: canGetCost canGetGradient getCost getGradient

% This file is part of Manopt: www.manopt.org.
% Original author: Nicolas Boumal, Dec. 30, 2012. Contributors:  Change log:
%   April 3, 2015 (NB):  Works with the new StoreDB class system.
%   Aug. 2, 2018 (NB): The value of the cost function is now always cached.
%   Sep. 6, 2018 (NB):  The gradient is now also cached.
...
% Contrary to most similar functions, here, we get the store by default. This is for the caching functionality described below.
store = storedb.getWithShared(key);   store_is_stale = false;
...
% If we get here, the cost was not previously cached, but maybe the gradient was?
if force_grad_caching && isfield(store, 'grad__')
    grad = store.grad__;
...
if isfield(problem, 'costgrad')
    ...
    % The two following calls will already cache cost and grad, then  the caches will be overwritten at the end of this function, with
