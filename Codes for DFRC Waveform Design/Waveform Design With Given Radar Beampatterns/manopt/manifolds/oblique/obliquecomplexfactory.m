...
%
% By default, columns have unit norm (product of m spheres in C^n). This can also be expressed with the flag 'cols'.
% 
% If that flag is set to 'rows', then the rows have unit norm instead (product of n spheres in C^m).
% 
% The metric is such that the oblique manifold is a Riemannian submanifold of the space of nxm matrices with the trace inner product (Frobenius):
%   <A, B> = real(trace(A'*B))
%
% Effectively, this treats C^n as R^(2n), with the real and imaginary parts treated separately as 2n real coordinates.
%
% Set gpuflag  to 'gpu' to have points, tangent vectors and ambient vectors stored on the GPU. If so, computations in cost, grad etc. can be done on
% the GPU directly. By default, the GPU is not used. This can also be expressed with the flag 'nogpu'.
%
% For backward compatibility, the two input flags can also be boolean:
%   obliquecomplexfactory(n, m, false)  == obliquecomplexfactory(n, m, 'cols')
%   obliquecomplexfactory(n, m, true)   == obliquecomplexfactory(m, n, 'rows') (mind m <-> n)
%   gpuflag: true == 'gpu' and false == 'nogpu'.
%   
% Note: obliquecomplexfactory(n, 1, 'rows') is equivalent to (but potentially slower than) complexcirclefactory(n).
%
% See also: spherecomplexfactory complexcirclefactory obliquefactory

% This file is part of Manopt: www.manopt.org. Original author: Nicolas Boumal, Sep. 3, 2014. Contributors: Change log:
%
%   Oct. 21, 2016 (NB)   Formatted for inclusion in Manopt release.
%
%   July 20, 2017 (NB)  Distance function is now accurate for close-by points. See notes  inside the spherefactory file for details. Also improves distances
%       computation as part of the log function.
%
%   May 28, 2023 (NB)      Fixed bug in M.log in case 'transposed' is true (bug reported by  Lingping Kong).
%
%   June 26, 2024 (NB)
%       Revamped to match the new format of the real version of this factory. In the process, we get GPU support. Backward compatible.
    ...
    % If gpuflag is active, new arrays (e.g., via rand, randn, zeros, ones)  are created directly on the GPU; otherwise, they are created in the
    ...
    function PXH = projection(X, H)
        % Compute the inner product between each column/row of H  with the corresponding column/row of X.
        inners = real(sum(conj(X).*H, dim));
        % Remove from H the components that are parallel to X, by row/col.
        PXH = H - X.*inners;
    end
    M.tangent = M.proj;
    % For Riemannian submanifolds, converting a Euclidean gradient into a Riemannian gradient amounts to an orthogonal projection.
